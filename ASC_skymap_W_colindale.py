...
#Colindale
colindale   = (earth + wgs84.latlon((51+35/60+44/3600),-(0+15/60+0/3600)),\
               51.59559,-0.25007,'51:35:44','N','00:15:00','W')

Obs         = colindale #<= set your observatory
   ...
   # others linecollection       
    ...
    for x,y,z,n,c in constellation_line:
        if math.hypot(numpy.mean(x),numpy.mean(y)) < hori_border/2 and max(x)-min(x) < hori_border:
            if n in set(['$\u2652$','$\u2648$','$\u264B$','$\u2651$','$\u264A$','$\u264C$','$\u264E$','$\u2653$','$\u2650$','$\u264F$','$\u2649$','$\u264D$']):
                ax0.annotate(str(n),(numpy.mean(x),numpy.mean(y)-labelxy),horizontalalignment='center',alpha=0.75,color='y',path_effects=[pe.withStroke(linewidth=1,foreground='k',alpha=0.75)],zorder=9+2.5)
                ax0.annotate(str(c),(numpy.mean(x),numpy.mean(y)-labelxy-12),fontproperties=chara_chi,ha='center',alpha=0.75,color='y',path_effects=[pe.withStroke(linewidth=1,foreground='k',alpha=0.75)],zorder=9+2.5)
##                #outline
...
##                ax0.annotate(str(c),(numpy.mean(x)-olw,numpy.mean(y)-labelxy-12+olw),fontproperties=chara_chi,ha='center',alpha=0.75,color='k',zorder=8+2.5)
                ...
                ax0.annotate(str(n),(numpy.mean(x),numpy.mean(y)-labelxy),horizontalalignment='center',alpha=0.75,color='w',path_effects=[pe.withStroke(linewidth=1,foreground='k',alpha=0.75)],zorder=9+2.5)
                ax0.annotate(str(c),(numpy.mean(x),numpy.mean(y)-labelxy-12),fontproperties=chara_chi,ha='center',alpha=0.75,color='w',path_effects=[pe.withStroke(linewidth=1,foreground='k',alpha=0.75)],zorder=9+2.5)
##                #outline
##                ax0.annotate(str(n),(numpy.mean(x)+olw,numpy.mean(y)-labelxy-olw),horizontalalignment='center',alpha=0.75,color='k',zorder=8+2.5)
##                ax0.annotate(str(n),(numpy.mean(x)+olw,numpy.mean(y)-labelxy),horizontalalignment='center',alpha=0.75,color='k',zorder=8+2.5)
##                ax0.annotate(str(n),(numpy.mean(x)+olw,numpy.mean(y)-labelxy+olw),horizontalalignment='center',alpha=0.75,color='k',zorder=8+2.5)
...
def write_label(x,y,z,c1,c2,c3): # (x,y) are text offset, z is zorder, (c1,c2,c3) are text colors
    ...
    ax0.annotate('科林達站',(twli_xmax+x,twli_ymax+y),ha='right',va='top',fontproperties=chara_chi,fontsize=10,color=c1,zorder=12+2.5+z)
    ax0.annotate('Colindale\nTube Station',(twli_xmax+x,twli_ymax+y-10),ha='right',va='top',color=c1,zorder=12+2.5+z)
...
#     CC_hokoon['HKT'] = pandas.to_datetime(CC_hokoon['HKT'],yearfirst=True) # convert to datetime object
    ...
    plt.savefig('Hokoon_skymap_W_colindale.png')
    ...
    ULdropbox = rclone.with_config(cfg).copy('/home/pi/Desktop/Hokoon_skymap_W_colindale.png','webpage:webpage')
    
    #upload to Oracle Cloud server
    ssh_client = paramiko.SSHClient()
    ssh_client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    try:
        ssh_client.connect(hostname='srv01.ajitama.logeebox.com',username='thomas',key_filename='/home/pi/Desktop/rsa_thomas.privatekey')

        timelog('SSH connected')

        sftp = ssh_client.open_sftp()
        sftp.put('/home/pi/Desktop/Hokoon_skymap_W.png', '/docker_data/buscountdown/v002/app/static/Hokoon_skymap_W_colindale.png')
        sftp.close()
        timelog('SSH uploaded')
